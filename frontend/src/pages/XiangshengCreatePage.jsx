...
import { createXiangshengStage1, createXiangshengStage2, createXiangshengStage3, fetchXiangshengDimensions } from '../api'
...
export default function XiangshengCreatePage() {
  ...
  const handleGenDialogue = async () => {
    ...
    try {
      const dims = {}
      if (toneStyle && toneStyle !== '通用') {
        dims.toneStyle = toneStyle
      }
    ...
    <div className="xs-page">
      ...
      {step === 1 && (
        <div className="xs-section-card">
          ...
          {/* 语气风格（唯一保留的维度，可选） */}
          {dimensions.filter(d => d.key === 'toneStyle').map(dim => (
            <div key={dim.key}>
              ...
              <div className="xs-style-grid">
                {dim.options.map(opt => (
                  <div
                    key={opt.value}
                    className={`xs-style-card ${toneStyle === opt.value ? 'active' : ''}`}
                    onClick={() => setToneStyle(toneStyle === opt.value ? '' : opt.value)}
                  >
